...
int main(int argc, char **argv) {
    ...
    foreach_phdr(map_start, &printer , fd);
    ...
    foreach_phdr(map_start, &printl, fd);
    foreach_phdr(map_start, &load_phdr, fd);
    return startup(argc, argv, (void*)header -> e_entry);
}

int foreach_phdr(void *map_start, void (*func)(Elf32_Phdr *,int), int arg) {
    header = (Elf32_Ehdr*) map_start;
    for (int i = 0 ; i < header -> e_phnum ; i++) {
        func((Elf32_Phdr *)(map_start + header -> e_phoff + i * header -> e_phentsize) , arg);
    }
    return 0;
}
...
void load_phdr(Elf32_Phdr *phdr, int fd) {
    if(phdr->p_type == PT_LOAD) {
        ...
        if (mmap(vaddr, phdr -> p_memsz + padding, phdr -> p_flags, MAP_PRIVATE | MAP_FIXED, fd, offset) < 0) {
            perror("Failed to create a new mapping");
            exit(1);
        }
        printl(phdr, fd);
    }
